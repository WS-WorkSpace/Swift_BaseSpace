...
class NineGridFlowLayout: UICollectionViewFlowLayout {
    ...
    override func prepare() {
        super.prepare()
        self.register(ImageReusableView.self, forDecorationViewOfKind: ImageReusableView.CID)
        ...
        for section in 0 ..< sectionCount {
            ...
            //                delegate.collectionView(collectionView: self.collectionView, layout: self, insetForSectionAt: section)
            //                }
            // 获取除了 section 间距外的矩形大小
            // CGRectUnion 获取包含两个 frame 的最小的矩形 frame
            var frame: CGRect = CGRectUnion(firstItemAttr.frame, lastItemAttr.frame)
            ...
            if self.scrollDirection == UICollectionView.ScrollDirection.horizontal {
                // 水平滚动，高度是固定的，宽度不固定
                frame.size.height = self.collectionView?.frame.size.height ?? 0.0
                frame.size.width += (sectionInset.left + sectionInset.right)
            } else { // 竖直滚动，宽度是固定的，高度不固定
                frame.size.height += (sectionInset.top + sectionInset.bottom)
                frame.size.width = self.collectionView?.frame.size.width ?? 0.0
            }
            // 装饰视图(类似于背景视图，如果做一个书架，该 View 可以做书架) 对于装饰视图的 UI 更新，需要覆写自定义的 UICollectionReusableView 的 applyLayoutAttributes 方法进行覆写
            let bgAttrbutes = UICollectionViewLayoutAttributes(forDecorationViewOfKind: ImageReusableView.CID, with: IndexPath(item: 0, section: section))
            // 放在最底层
            bgAttrbutes.zIndex = -1
            bgAttrbutes.frame = frame
            self.itemAttributes.append(bgAttrbutes)
        }
    }
}
