//
//  OC_EmptyDataSetTableViewController.swift
//  Swift_Base
//
...
class OC_EmptyDataSetTableViewController: UIViewController {
    fileprivate var bookArray = [Book]()
    lazy var mTableView: EmptyBaseTableView = {
        let tabview = EmptyBaseTableView()
...
//        mTableView.title(forEmptyDataSet: <#T##UIScrollView!#>)
        mTableView.showEmptyDataViewWithType(.StateNetWorkError)
        mTableView.ClickEmptyViewBlock = { [weak self] in
            self?.getNetModel()
        }
...
extension OC_EmptyDataSetTableViewController: UITableViewDataSource {
...
extension OC_EmptyDataSetTableViewController: UITableViewDelegate {
